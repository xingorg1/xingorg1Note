...
### 路径获取
#### 1、文件拖拽的方式
要找到你的文件地址，只需要把你要打开的文件直接拖到编辑器终端的窗口（或电脑自带的命令行）里，就会显示出文件地址，然后复制一下就好。拖到终端获取的地址是绝对路径。
![路径获取小知识](./images/getDir1.png)
出现路径：
![路径获取小知识](./images/getDir2.png)
#### 2、VScode里的快捷选项
用VS Code打开文件夹，在文件点击右键，选择：
![路径获取小知识](./images/getDir3.png)
#### 3、pwd命令
mac电脑中，还可以在命令行中使用`pwd`获取当前cmd打开地址的路径。
...
r| 第二个参数表示打开文件时的模式。这里是字符串 'r'，表示 read，表示我们以读的模式打开了这个文件。
...
### 读取
#### 用`read()`函数将整个文件一次性读取出来的操作
```py
readResult = openResult.read() # 文件读取后的内容赋值给readResult变量，类型是字符串的数据形式
...
参数|说明
-|-
w| 第二个参数表示打开文件时的模式。这里是字符串 'w'，表示 write，即以写入的模式打开文件。不过值得注意的是，‘w’是覆盖式写入，新write的内容会覆盖该文件原有的内容。表现上就是先清空文件内容，再写入。
a| 表示append，顾名思义，就是向后追加的意思。其和write作用一样都是向文件中写入的模式。不过append更人性化的是，向原有文件的内容之后追加内容，而不是直接覆盖源文件的内容。
**在'w'和'a'模式下，如果你打开的文件不存在，那么open()函数会自动帮你创建一个**
### 写入
#### 使用`write()`函数往文件中写入一个内容
```py
openResult.write('小石头\n') # write参数必须是字符串
openResult.write('xing.org1^\n') # 向openResult中写入内容。参数就是要写入的内容，是字符串形式文本内容。\n表示换行符。
print(openResult) # 可以打印出写入后的文本信息
```
...
#### 使用`writelines()`函数向文本中批量写入内容
```py
listVal = ['小石头\n', 'xing.org1^\n'] # 要换行还要自己加入换行符
openResult.writelines(listVal) # writelines参数传一个序列
```
...
## open函数
### 打开文件的模式
参照上边的代码，open函数调用时，第二个参数就需要传入是r还是w亦或者是其他模式。
...
a|append|追加模式|在源文件内容的基础上<br/>追加写入新内容|文件不存在新建，<br/>存在指针放到末尾（追加）
read文件不存在就报错
![read文件不存在就报错](./images/fileReadError.png)
### 二进制资源读写
w和a写入的都是文本内容。

如果想写入图片、视频、音频等这些以二进制模式进行存储的资源内容时，需要使用‘wb’这些以二进制的方式打开并读写文本的模式。
...
练习目标是读取文件后计算三人的总成绩。然后写到demo文件下，格式如：
```md
<!-- 修改后，demo.txt文件内容 -->
小石头 100 93 83
郭菊锋 89 67 9
...
[源代码](https://github.com/xingorg1/xingorg1Note/blob/master/docs/backEnd/python/files/17-file.py#L37)
